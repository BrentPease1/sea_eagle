...
# cleaned data file
if(!(file.exists(here('Data/cleaned_responses/sea_eagle_cleaning_2023_01_31_geocoded_travel_cost.csv')))){
  ...
  # bring in flight info
  flights <- read.csv(here('Data/flight_distances.csv'))
  clean_data <- merge(clean_data, flights, by = 'respond_id', all.x = T)
  clean_data <- clean_data %>%
    mutate(flight_time_sec = flight_time * 60)
  
  # replace all driving travel distances calculated for flyers with flight travel time
  clean_data$travel_time_sec[!is.na(clean_data$flight_time_sec)] <- clean_data$flight_time_sec[!is.na(clean_data$flight_time_sec)]
  ...
  # use reported hourly wage if available otherwise use federal rate
  clean_data <- clean_data %>%
    mutate(hourly_wage = as.numeric(hourly_wage))
  clean_data <- clean_data %>%
    mutate(hourly_wage = ifelse(is.na(hourly_wage), hourly_wage_rate, hourly_wage))
  
  
  clean_data <-  clean_data %>%
    mutate(automobile_operating_cost = ((((travel_dist_meter * 0.00062137)*2) * standard_auto_cost) / 100),
           opp_cost_time = (travel_time_sec / 3600) * (0.5 * hourly_wage),
  ...
  write.csv(clean_data, file = here("Data/cleaned_responses/sea_eagle_cleaning_2023_01_31_geocoded_travel_cost.csv"))
  
  
} else{
  
  clean_data <- read.csv(here('Data/cleaned_responses/sea_eagle_cleaning_2023_01_31_geocoded_travel_cost.csv'))
